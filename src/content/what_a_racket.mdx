...
<MyPop text="Surely this joke isn't overused.">A self-referencing function is a function that refers to itself.</MyPop> It'll operate on a list that itself contains more lists (e.g. ListOfColor is either `empty` or `cons Color ListOfColor`). Through the principle of induction, we can be certain that the sub-lists will yield to the function just the same as their parent lists, until they arrive at some base case. These lists are horizontal.
